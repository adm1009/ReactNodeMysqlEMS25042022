...
> {
  ...
  forgotHandler = () => {
    this.setState({ forgotshow: true, login: false,birthdayshow:false, loginstatus: "" });
  };
  ...
  render() {
    return (
      <>
        ...
        <section style={{display:"inline-block",position:"absolute",marginLeft:"15px"}}>
        {!this.state.show && !this.state.forgotshow && (
          <h4 onClick={this.birthdayshowHandler} style={{fontSize:"15px"}}>Birthdays</h4>
        )}
        {this.state.birthdayshow && !this.state.show && (
          <div style={{textAlign:"center"}}>
            <section>
              <h3 style={{ color: "brown" }}>Todays Birthday</h3>
              <table>
                <tbody>
                  {this.state.birthdayList.map((value: any) => {
                    ...
                    else if (!this.state.birthdayList) {
                      return (
                        <td>
                          <BsEmojiFrownFill style={{ color: "yellow" }} />
                          No Birthdays Today
                        </td>
                      );
                    }
            ...
            <section>
              ...
              <table>
                <tbody >
                  {this.state.birthdayList.map((value: any) => {
                    if (value.month === new Date().getMonth() + 1 && value.date > new Date().getDate())
                      return (
                        <tr key={value.id}>
                          <td>
                            <GiBalloons style={{ color: "red" }} />
